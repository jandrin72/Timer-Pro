    ...
    let restoreMasterGainTimeout = null;
    function victoryBells() {
      const audio = new Audio('src/sounds/final fanfare.wav');

      const requestedVolume = (typeof AudioUtil !== 'undefined' && typeof AudioUtil.getVolume === 'function')
        ? AudioUtil.getVolume()
        : 1.0;
      audio.volume = Math.max(0, Math.min(1, requestedVolume || 1.0));

      if (restoreMasterGainTimeout) {
        clearTimeout(restoreMasterGainTimeout);
        restoreMasterGainTimeout = null;
      }
      ...
      ensureAudioContext().then(ctx => {
        ...
        restoreMasterGainTimeout = setTimeout(() => {
          const desired = (typeof AudioUtil !== 'undefined' && typeof AudioUtil.getVolume === 'function')
            ? AudioUtil.getVolume()
            : requestedVolume;
          const currentTime = ctx.currentTime;
          masterGain.gain.cancelScheduledValues(currentTime);
          masterGain.gain.setValueAtTime(Math.max(0, Math.min(1, desired || 1.0)), currentTime);
        }, 1200);
      }).catch(() => {});

      audio.play().catch(e => console.warn('Error reproduciendo sonido de victoria:', e));
    }
